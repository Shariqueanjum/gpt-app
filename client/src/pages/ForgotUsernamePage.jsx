// ============================================================
// ForgotUsernamePage.jsx — Request username via email (MODAL)
// API: POST /api/auth/forgot-username { email }
// ============================================================
...
const ForgotUsernamePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      await axiosInstance.post('/auth/forgot-username', { email: email.trim().toLowerCase() })
      setSuccess(true)
    } catch (err) {
        ...
        {success ? (
          <div className="text-center py-6">
            ...
            <p className="text-[#494454] text-sm mb-8 leading-relaxed">
              If an account exists with <span className="font-semibold text-[#131b2e]">{email}</span>, we have sent your username to your email.
            </p>
            <button
              onClick={handleClose}
          ...
          <>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              >
                {loading ? <span className="inline-block w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Send Username'}
              </button>
            </form>
